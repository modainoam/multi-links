...
function openLinks(type) {
    ...
    $(typeClass + ".form-check-input:checked").each(function() {
        ...
        // Allow for centralOps post request function
        if (check_id !== "central-ops") {
            linkArray.push(getLink(check_id, domain_name))
        } else {
          openCentralOps(domain_name);
        };
    });
...
function openCentralOps(domain_name) {
// Make a post request with said variable to the specified site.
    $(".new_window_post[name=addr]").val(domain_name);
